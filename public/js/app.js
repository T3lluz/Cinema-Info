...
const I18N = {
  nb: {
    ...
    ratingMetacriticAria: "Metacritic {n} av 100",
  ...
  en: {
    ...
    ratingMetacriticAria: "Metacritic {n} out of 100",
...
function setupDaySwipe() {
  ...
  host.addEventListener("pointermove", (e) => {
    ...
    if (dt > 0 && dt < 48) {
      ...
      vx = vx === 0 ? inst : inst * 0.55 + vx * 0.45;
      lastX = x;
      lastT = e.timeStamp;
    } else if (dt >= 48) {
      vx = 0;
...
function ratingLogo(kind, score) {
  ...
  // Official three-dot mark — orange / green / blue.
  if (kind === "letterboxd") {
    return `<svg class="rating-logo letterboxd" viewBox="0 0 24 16" aria-hidden="true"><circle cx="6" cy="8" r="5.15" fill="#ff8000"/><circle cx="12" cy="8" r="5.15" fill="#00e054"/><circle cx="18" cy="8" r="5.15" fill="#40bcf4"/></svg>`;
  }
  if (kind === "metacritic") {
...
function renderRatingBadges(ratings) {
  ...
  if (ratings.metacritic?.value != null) {
    const score = Math.round(Number(ratings.metacritic.value));
    if (Number.isFinite(score)) {
      const label = String(score);
      const body = `${ratingLogo("metacritic", score)}<span class="rating-value">${escapeHtml(label)}</span>`;
      const aria = t("ratingMetacriticAria", { n: label });
      bits.push(
        ratings.metacritic.url
          ? `<a class="rating-badge metacritic" href="${escapeHtml(ratings.metacritic.url)}" target="_blank" rel="noopener" aria-label="${escapeHtml(aria)}">${body}</a>`
          : `<span class="rating-badge metacritic" role="img" aria-label="${escapeHtml(aria)}">${body}</span>`
      );
    }
  }
...
function renderPopularCard(item, index = 0) {
  ...
  const metaVal = Number.parseInt(movie.metascore, 10);
  if (Number.isFinite(metaVal)) {
    ratings.metacritic = { value: metaVal };
  }
...
function programRatingsForMovie(movie) {
  if (!state?.shows) return null;
  const id = String(movie?.imdbID || "").trim();
  const title = String(omdbSheetInProgram || movie?.title || "")
    .trim()
    .toLowerCase();
  const show = state.shows.find((s) => {
    if (id && String(s.ratings?.imdb?.id || "") === id) return true;
    return title && String(s.title).toLowerCase() === title;
  });
  return show?.ratings || null;
}
...
function omdbRatingsFromMovie(movie) {
  ...
  const imdbVal = Number.parseFloat(movie.imdbRating);
  if (Number.isFinite(imdbVal)) {
    fromOmdb.imdb = { value: imdbVal, url: movie.imdbUrl || "" };
  }
  const metaVal = Number.parseInt(movie.metascore, 10);
  if (Number.isFinite(metaVal)) {
    fromOmdb.metacritic = { value: metaVal };
  }
  for (const row of movie.ratings || []) {
    ...
    if (/rotten/i.test(source)) {
      ...
      if (Number.isFinite(n)) fromOmdb.tomatoes = { value: n };
    } else if (/metacritic/i.test(source)) {
      const n = Number.parseInt(raw, 10);
      if (Number.isFinite(n)) fromOmdb.metacritic = { value: n };
    } else if (/imdb|internet movie/i.test(source) && fromOmdb.imdb == null) {
      const n = Number.parseFloat(raw);
      if (Number.isFinite(n)) {
        fromOmdb.imdb = { value: n, url: movie.imdbUrl || "" };
      }
    }
  }
  const badges = renderRatingBadges(
    mergeRatingSources(fromOmdb, programRatingsForMovie(movie))
  );
