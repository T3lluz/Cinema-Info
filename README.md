...
IMDb rating + genres are looked up via IMDb’s suggestion search and
GraphQL cache (no API key). An optional `OMDB_API_KEY` is still
accepted as a fallback if the direct lookup misses.
...
`OMDB_API_KEY=…` is optional — only used if the direct IMDb lookup
misses a title.
