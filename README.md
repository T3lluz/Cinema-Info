...
1. `scripts/fetch-data.mjs` snapshots the programme into `public/data/program.json`. Buen's website feed is the source for posters, tags and upcoming titles; **DX's public cinema listing** fills in every film showing in the 120-day window — including days Buen has not published yet, and history Buen drops the moment a show starts.
2. `.github/workflows/refresh-program.yml` runs that script twice a day and commits when the schedule changed
3. The phone site live-updates **sold counts + real end times** from the DX/eBillett API on its five-second beat, and every couple of minutes it also walks DX's cinema listing so a day added in the till appears without waiting for the next snapshot
4. With a connected DX account, it also pulls **scanned ticket** counts, for past days as well

The Movies tab shows **IMDb**, **Letterboxd** and **Rotten Tomatoes
audience** scores (with logos) when the snapshot could find them.
Genres come from IMDb (English source names, translated in the UI).
Letterboxd and Rotten Tomatoes are read from their public film pages.
IMDb rating + genres prefer an `OMDB_API_KEY` (free at
[omdbapi.com](https://www.omdbapi.com/apikey.aspx)) — add it as a
**repository Actions secret** named `OMDB_API_KEY` (Settings → Secrets
...
it starts, so the script also walks DX's cinema catalogue back over the
keep-window (120 days) — that keeps sold counts, posters and DX event
ids alive for days the website no longer lists. Run it by hand any time
with `node scripts/fetch-data.mjs`.
...
```
public/                 # what you serve / what Pages publishes
  index.html
  sw.js
