...
Genres come from IMDb via OMDb (English source names, translated in the
UI). Letterboxd and Rotten Tomatoes are read from their public film
pages; IMDb rating + genres need an
`OMDB_API_KEY` (free at [omdbapi.com](https://www.omdbapi.com/apikey.aspx))
passed into the fetch script / Actions secret of the same name.
...
```bash
OMDB_API_KEY=… node scripts/fetch-data.mjs
